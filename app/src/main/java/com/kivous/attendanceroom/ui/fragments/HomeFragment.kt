...
//        askForNotificationPermission()
        ...
        binding.vProfile.setOnClickListener {
    ...
    private fun setUpRecyclerView() {
        lifecycleScope.launch {
            viewModel.classRoomFirestoreRecyclerOptions.collectLatest {
                when (it) {
                    is Response.Loading -> {
                    ...
                    is Response.Success -> {
                        ...
                        try {
                            adapter = it.data?.let { it1 ->
                                ClassRoomAdapter(
                                    it1,
                                    ::classRoomAdapterViewController
                                )
                            }!!
                        } catch (e: Exception) {
            ...
            if (userEmail in model.teacherEmailList!!) {
                holder.apply {
                    binding.apply {
                        ...
                        if (studentCount == 1) {
                            binding.tvCreator.text = "$studentCount student"
                        } else {
                            binding.tvCreator.text = "$studentCount students"
                        }
                                            ...
                                            R.id.two -> {
                                                if (isConnected) {
                                                    MaterialAlertDialogBuilder(
                                                        requireContext()
                                                    ).setTitle("Archive class?")
                                                        .setMessage("You and your students won't be able to make changes.\n\nYou can view this class in Archived classes.")
